...
const createTask = async (req, res) => {
  ...
  if (taskExists) {
    res.status(401).json({
      errorMessage: "Task name already exists! Please choose another name.",
      status: false,
    });
  } else {
    //creating an object
...
const getEmp = async (req, res) => {
  ...
  const teamData = await Team.findById(projectData.team).populate({
    path: "members",
    select: { _id: 1, fullName: 1, designation: 1 },
  });
...
const getTaskEmp = async (req, res) => {
  const task = await Task.find({ assignee: req.user.id }).populate(
    "projectId",
    "projectName"
  );
  ...
  await Task.find({ assignee: req.user.id })
...
const completeTask = async (req, res) => {
  const { stage, completedDate, dueDate, credits, userId } = req.body;
...
const requestExtention = async (req, res) => {
  const { isRequested, taskName, assignee } = req.body;
