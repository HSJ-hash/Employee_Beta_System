...
const deleteProject = async (req, res) => {
  try {
    ...
    if (project) {
      // Delete vacancies with the removed project id
      await Vacancy.deleteMany({ project: req.params.id });
      await Task.deleteMany({ projectId: req.params.id });

      res.status(200).json({
        data: "Project and associated vacancies deleted",
        status: true,
      });
    } else {
      res.status(401).json({
...
const homeData = async (req, res) => {
  ...
  const notice = await Document.find();
...
const getBurndownDetails = async (req, res) => {
  ...
  if (projectData) {
    ...
    var timeDiff =
      projectData.deadline.getTime() - projectData.startDate.getTime();
