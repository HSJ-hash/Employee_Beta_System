...
export default function Employees() {
  ...
  const [fetEmp, fetchedEmps] = React.useState([]);
  ...
  async function getToken(UID) {
    console.log(UID);
    await axios.post(`http://localhost:8070/user/token/${UID}`).then((res) => {
          ...
          <TableHead>
            <TableRow>
              ...
              <TableCell className="col-md-1" align="center">
                <p className="tbHeading">Options</p>
              </TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            {filteredEmps.map((row) => (
              ...
              >
                ...
                <TableCell component="th" scope="row" className="col-md-1">
                  <div className="text2" style={{ display: "flex" }}>
                    {/*Edit BTN*/}

                    <IconButton
                      onClick={() => {
                        EmpIdFetchupd(row._id);
                      }}
