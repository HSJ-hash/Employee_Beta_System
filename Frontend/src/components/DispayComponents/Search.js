...
export default function Search() {
  ...
  const filterVacancy = (data) => {
    const filtData = Vacancy.filter((item) => {
      return item.vacancy.toLowerCase().includes(data.toLowerCase());
    });
    if (filtData.length !== 0) {
      setShowVacancy(true);
    } else {
      ...
      <Tooltip title="Search">
        <IconButton
          sx={{
            ...
            width: '40px', // Optional: Match width for consistent sizing
            display: 'flex',
            alignItems: 'center',
      ...
      >
        ...
        <DialogContent className="p-0">
          <DialogContentText id="alert-dialog-description">
            ...
            {showVacancy && search !== "" && (
              <div>
                <p className="searchTopic">Vacancy</p>
                <MenuList className="p-3">
                  {filteredVacancy.map((task) => (
                    <div>
                      <MenuItem
                        className="listItem"
                        sx={{
                        ...
                        onClick={() => {
                          handleClose();
                          navigate("/vacancies/viewVacancy/" + task._id);
                        }}
                      >
                        <ListItemIcon>
                          <TaskIcon
                        ...
                        <ListItemText>
                          <p className="listText">{task.vacancy}</p>
                          <p className="listSubText">
                            {/* {task.projectId.projectName} */}
                          </p>
                        </ListItemText>
